...
import './BuscaEventoNome'
import './Eventos.css';
import './BuscaEventoNome.css'

export default function BuscaEventosNome() {
    ...
    // Função para lidar com a submissão do formulário de busca de eventos por nome
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await fetch(`http://localhost:8080/Eventos/get-evento-nome?nome=${nome}`);
            if (response.ok) {
                const data = await response.json();
                setEventos(data);
            } else {
                setEventos([]);
                console.error('Erro ao buscar eventos por nome:', response.statusText);
                setError('Erro ao buscar eventos por nome. Tente novamente mais tarde.');
            }
        } catch (error) {
        ...
        try {
            ...
            if (response.ok) {
                const updatedEventoWithTipo = {
                    ...updatedEvento,
                    tipo: tiposDeEvento.find(tipo => tipo.id === parseInt(updatedEvento.tipoId)).nome
                };
    ...
    return (
        <>
            ...
            <div className="header_evento">
                <h1>Busca de Eventos por Nome</h1>
            </div>
